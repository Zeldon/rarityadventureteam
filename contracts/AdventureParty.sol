...
contract AdventureParty is Ownable, IERC721Receiver {
    ...
    constructor(address _rarity, address _raritygold) {
        rarity = Rarity(_rarity);
        rarityGold = RarityGold(_raritygold);
    ...
    function adventureAll() external {
        for (uint i = 0; i < adventurers.length(); i++) {
            ...
            // checks for if adventure can be done.
    ...
    function registerAdventurers(uint[] memory summonerIDs) external onlyOwner {
        for (uint i = 0; i < summonerIDs.length; i++) {
            // deleting owner check to register for adventures
    ...
    // add unregister all button
    function unregisterAll() external {
        for (uint i = 0; i < adventurers.length(); i++) {
            adventurers.remove(adventurers.at(i));
        }
    }
